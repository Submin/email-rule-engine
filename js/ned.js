...
Ned.Node = {
	...
	addInput(item) {
		return this.addConnection (item, true);
	},

	addOutput(item) {
		return this.addConnection (item, false);
	},

	addConnection (item, isInput) {
		var conn = Object.create (Ned.Connector);
		if (isInput) this.inputs.push(conn);
		else this.outputs.push(conn);

		conn.init(this, item, isInput);
		this.updateVisuals();
...
Ned.Connector = {
	init(node, item, isInput) {
		...
		this.eText.appendChild(document.createTextNode(item.name));
        this.eTitle = document.createElementNS(this.editor.svg.ns, "title");
        this.eTitle.appendChild(document.createTextNode(item.desc));
		this.eRoot.appendChild(this.eText);
		this.eRoot.appendChild(this.eTitle);
...
//         // Every outpu must have single connection only if dragged from output
//         if (!this.isInput && this.paths.length >= 1) {
//             path.destroy();
// 			return false;
//         }
//         // Every outpu must have single connection only if dragged to output
//         if (!conn.isInput && conn.paths.length >= 1) {
//             path.destroy();
// 			return false;
//         }
	...
	beginConnDrag(e) {
		...
		if (!this.isInput && this.node.outputs.length > 1) {
            return;
        }
